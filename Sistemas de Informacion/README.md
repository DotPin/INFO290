 
# Disciplina a desplegar

**Sistemas de Información**

***

## Descripción

Disciplina cual permite la descripción y desglose de necesidad para elaboración de sistemas, donde cada proceso de elaboración de modelos para sus diseños sistemáticos, son generados a partir de requerimientos adquiridos por el negocio en cuestión. Los modelos aplicados o usados para la generación de sistemas entregan una idea de negocio cual proyecta funcionamiento en su estado natural, tras esto, su formulación mediante diagramas, sistematiza su funcionar para posteriores mejoras a nivel tecnológico.

## Propuestas

- Caso de Uso 1: Modelamiento de negocio para idea de sistematización.


## Documentos:

- [Documentación](https://github.com/DotPin/INFO290/tree/master/Sistema%20de%20Gesti%C3%B3n%20Estrat%C3%A9gicos/Documentacion): Material de respaldo para Casos de Estudio.

- [Caso Estudio 1](https://github.com/DotPin/INFO290/tree/master/Sistema%20de%20Gesti%C3%B3n%20Estrat%C3%A9gicos/Tarea%201): Modelamiento de negocio para sistematización.


## Referencias:

- Information Systems Development: an introduction to information systems engineering., Beynon-Davies, P. (1998)

- The complete business process handbook: Body of knowledge from process modeling to BPM, volume 1 (Vol. 1). Morgan Kaufmann, Von Rosing, M., Von Scheel, H., & Scheer, A. W. (2014). 

- Laudon, K., & Laudon, J. (2014). Management information systems . (13 ed., p. 648). Saddle River: Prentice Hall

- Laguna, M., & Marklund, J. (2013). Business process modeling, simulation and design . CRC Press.

- Wallace, P. (2015). Introduction to Information Systems . Pearson

- Rainer, R. K., Prince, B., & Watson, H. J. (2014). Management Information Systems . Wiley Publishing.