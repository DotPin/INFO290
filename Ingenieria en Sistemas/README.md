# Disciplina a desplegar

**Ingeniería en Sistemas**

***

## Descripción

Disciplina del área de la ingeniería, cual permite la creación de modelos que caracterizan ciclos de vida de sistemas de ingeniería complejos con altos grados de incertidumbre, cuales exigen un enfoque (inter/multi)disciplinario y no reduccionista para su diseño, desarrollo y su operación.

## Propuestas

- Caso de Uso 1: Desafío cual permite presentar un diseño procedimental de modelo unificación lógico (UML), para un proceso de venta.

## Documentos:

- [Solución](https://github.com/DotPin/INFO290/tree/master/Fundamentos%20de%20Ing.%20de%20Software/Solucion): Directorio con la presentación de caso de estudo procedimental UML.

- [Enunciado](https://github.com/DotPin/INFO290/blob/master/Fundamentos%20de%20Ing.%20de%20Software/Enunciado): Enunciado cual describre el desafío a realizar.

## Referencias:

- Walden, D. D., Roedler, G. J., Forsberg, K., Hamelin, R. D., & Shortell, T. M. (Eds.). (2015). Systems engineering handbook: A
guide for system life cycle processes and activities.

- De Weck, O. L., Roos, D., & Magee, C. L. (2011). Engineering systems: Meeting human needs in a complex technological
world. Mit Press.

- Skyttner, L. (2005). General systems theory: problems, perspectives, practice. World scientific.

- Hester, P. T. (2014). Systemic thinking: Fundamentals for understanding problems and messes. Cham: Springer.

- Sterman, J. D. J. D. (2000). Business dynamics: systems thinking and modeling for a complex world (No. HD30. 2
S7835 2000).

- Meadows, D. H., & Wright, D. (2008). Thinking in systems: A primer. chelsea green publishing.

- Hitchins, D. K. (2008). Systems engineering: a 21st century systems methodology. John Wiley & Sons.

- Eppinger, S. D., & Browning, T. R. (2012). Design structure matrix methods and applications. MIT press