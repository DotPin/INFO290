...
#define PASSWD "<CLAVE>"                   // Clave de tu red WiFi
#define HOSTFIREBASE "<URL_HOST_FIREBASE>" // Host o url de Firebase
#define SECRET_KEY "<API_KEY_FIREBASE>"
...
void loop()
{
  ...
  if (!stop1)
  {
    ...
    eventAction();
    Serial.println("Dice RUN el auto");
  }
  else
  {
    ...
    rpm = 0;
    aceleration = 0;
    Serial.println("Dice STOP el auto");
  }
...
void capturarStop()
{
  String path = / dispositivo / "+macStr+" / ";
                                             Serial.println(path);
  FirebaseObject object = Firebase.get(path);
  String d1 = object.getString("carld");
  Serial.println("Aquí el stop!");
  Serial.println(d1);
  if (d1 == "1.0")
  {
    stop1 = true;
