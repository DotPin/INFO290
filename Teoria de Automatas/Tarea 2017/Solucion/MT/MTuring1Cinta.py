...
class MTuring1Cinta:
    ...
    def aceptaPalabra(self, pala):
        palabra = list(pala)
        if(palabra[-1] != 'B'):
        	salida = "La palabra no contiene el simbolo blanco solicitado 'B'. favor corregir."
        	return salida
        	exit()
        correcto = True
        for i in palabra:
        ...
        if (correcto):
            ...
            while (estadoActual != self.final and transicion != "**************" and i < len(palabra)):  # mientras no se llegue al estado final, a una transicion vacia, o fin de la cadena
